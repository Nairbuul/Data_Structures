...
template <typename T>
void tree_insert(tree_node<T>* &root, const T& insert_me){
    //tree_search to keep the tree unique
...
void tree_print_debug(tree_node<T>* root, int depth, ostream& outs){
    if(root){
        ...
        if(!debug){
            ...
            outs << setw(4 + depth*6) << '[' << root->_item << ':' << root->_height 
        ...
        if(debug){
            ...
            tree_print_debug(root->_right, depth+1);
            cout << "Node: [" << root->_item << ']' << "\tHeight: " << root->_height << "\tLeft Child: " << boolalpha << null_left 
...
tree_node<T>* rotate(tree_node<T>* & root){
    if(root){
        ...
        if(balance_factor >= 2){
            if(root->_left && root->_left->balance_factor() <= -1){
        ...
        if(balance_factor <= -2){
            if(root->_right && root->_right->balance_factor() >= 1){
