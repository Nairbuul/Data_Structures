#ifndef LINKEDLISTFUNCTIONSTEMPLATE_H
#define LINKEDLISTFUNCTIONSTEMPLATE_H
...
T _delete_node(node<T>*& head, node<T>* delete_this){
    ...
    if(delete_this == head){ //Under the assumption the head node given is actually the beginning of the list.
        head = head->_next;  //Should be fine if last node cause it'll assign head to nullptr.     
    }
